...
// Force reseed function - clears and reseeds all dropdown data
async function forceReseedDropdowns() {
  try {
    console.log('[RESEED] Starting force reseed of all dropdown data...');
    ...
    console.log('[RESEED] Cleared all existing dropdown data');
    ...
    console.log('[RESEED] All dropdown data reseeded successfully!');
    return { success: true, message: 'All dropdown data reseeded successfully' };
  } catch (error) {
    console.error('[RESEED] Error reseeding dropdown data:', error);
    throw error;
  }
}
...
// Force reseed endpoint - protected by admin auth
app.post('/api/admin/reseed-dropdowns', adminAuthMiddleware, async (req, res) => {
  try {
    const result = await forceReseedDropdowns();
    res.json(result);
  } catch (error) {
    console.error('Error reseeding dropdowns:', error);
    res.status(500).json({ error: 'Failed to reseed dropdown data' });
  }
});
