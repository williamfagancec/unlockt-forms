...
const sanitizeFilename = (filename) => {
  ...
  const sanitized = basename
    .replace(new RegExp("[\\x00-\\x1f\\x7f]", "g"), "_")
    .replace(/["'<>|:*?\\/]/g, "_")
    .replace(/\r|\n/g, "_");

  if (sanitized.length === 0 || sanitized === "." || sanitized === "..") {
    throw new Error(
      "Invalid filename: filename contains only invalid characters",
    );
  }

  if (sanitized.length > MAX_FILENAME_LENGTH) {
    throw new Error(
      `Invalid filename: filename exceeds maximum length of ${MAX_FILENAME_LENGTH} characters`,
    );
  }
